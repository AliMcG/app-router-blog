...
const BlogListContainer = async ({ blogs }: { blogs: BlogPostPostgres[]}) => {
  ...
  return (
    ...
    {blogs.map((post, index) => {
      return (
        <div key={index} className="relative mx-4 mb-4 flex h-4/5 flex-col-reverse items-center rounded border bg-custom-blue p-4 hover:border-[#CFE1FF] lg:w-3/5 lg:items-start  lg:odd:flex-row-reverse lg:odd:justify-end lg:even:flex-row ">
        <div className="absolute rounded-full border-2 border-[#CFE1FF] bg-white odd:right-5 even:left-5 lg:bottom-5 ">
          <div>
            <span className="relative text-red-500">
              <HeartIcon className="h-8 w-8" />
            </span>
            <p className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform text-sm text-white">
              {post.numberVotes}
            </p>
          </div>
        </div>
